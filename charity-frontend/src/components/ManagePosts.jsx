...
const ManagePosts = () => {
  ...
  const handleEdit = (post) => {
    setEditingPost(post);
    setEditAmount(post.requiredAmount);
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setPosts(
        posts.map((post) =>
          post._id === editingPost._id ? response.data.updatedPost : post
        )
      );
  ...
  const handleDelete = async (postId) => {
    if (window.confirm("Are you sure you want to delete this post?")) {
      try {
        await axios.post("http://localhost:3030/deletePost", { postId });
                    ...
                    <span
                      className={`badge ${
                        post.status === "approved"
                          ...
                          ? "bg-danger"
                          : post.requiredAmount <= post.currentDonationsReceived
                          ? "bg-primary"
                          : "bg-warning"
                      }`}
                    >

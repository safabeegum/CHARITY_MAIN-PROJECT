...
const QuizLeader = () => {
  ...
  const fetchQuizLeader = async () => {
    try {
      const response = await fetch("http://localhost:3030/api/getQuizLeader");
      ...
      const sortedLeaders = data.quizLeaderboard.sort(
        (a, b) => b.score - a.score
      );
  ...
  return (
    <div>
      ...
      <div className="leaderboard-container" style={{ textAlign: "center" }}>
        ...
        {leaders.length > 0 && (
          <h4>
            🥇 Top Scorer: {leaders[0].userId?.username} with {leaders[0].score}{" "}
            points!
          </h4>
        )}

  ...
  const fetchHangmanLeader = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3030/api/getHangmanLeader",
        {
          method: "POST",
    ...
    <div>
      ...
      <div className="leaderboard-container" style={{ textAlign: "center" }}>
        ...
        {loading ? (
          ...
          <>
            ...
            <ResponsiveContainer width="90%" height={400}>
              ...
              >
                ...
                />
                <Tooltip />
                <Bar dataKey="score" fill="#FF7F50" barSize={50}>
                  {" "}
                  {}
                  {leaders.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={index === 0 ? "#FFD700" : "#FF7F50"}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <h4 style={{ color: "gray" }}>
            No leaders today! Play a game to be the first leader.
          </h4>
        )}
      </div>
    </div>

...
const Hangman = () => {
  ...
  const selectNewWord = () => {
    ...
    while (randomLetters.length < 2) {
      const randomLetter =
        chosenWord[Math.floor(Math.random() * chosenWord.length)];
  ...
  const saveScore = async (chancesUsed) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:3030/api/saveHangmanScore",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ score: chancesUsed }),
        }
      );

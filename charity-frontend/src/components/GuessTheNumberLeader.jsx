  ...
  const fetchGuessTheNumberLeader = async () => {
    try {
      const response = await fetch(
        "http://localhost:3030/api/getGuessTheNumberLeader"
      );
      const data = await response.json();

      const sortedLeaders = data.guessTheNumberLeader.sort(
        (a, b) => a.score - b.score
      );
  ...
  return (
    <div>
      ...
      <div className="leaderboard-container" style={{ textAlign: "center" }}>
        ...
        {leaders.length > 0 && <h4>🏆 Winner: {leaders[0].userId.username}</h4>}
          ...
          >
            ...
            <Bar dataKey="score" fill="#82ca9d" barSize={50}>
              {leaders.map((entry, index) => (
                <Cell
                  key={entry.userId.username}
                  fill={index === 0 ? "#FFD700" : "#82ca9d"}
                />
              ))}
            </Bar>
          </BarChart>
